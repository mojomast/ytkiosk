...
- **Keyword-based browsing** — caregivers add/edit/remove topics (e.g. "Nature documentaries", "Classic cars", "Animal compilations"). Keywords wrap into new columns instead of requiring scrolling
...
- **Language and favorites** — EN/FR toggle and a personal favorites playlist
...
```bash
curl -fsSL 'https://raw.githubusercontent.com/mojomast/ytkiosk/main/install.sh?v=2026-06-21.7' | YTKIOSK_INSTALL_DENO=1 bash
```
...
Use the ✎ button inside the app to edit keywords, click **×** to remove one, or click **+ Ajouter un mot-clé** to add a new one. Keywords save automatically. Caregivers can disable keyword adding/editing/removal from the password-protected **⚙ Options** menu. You can also edit `INITIAL_KEYWORDS` in `src/ytkiosk/legacy.py` to set the defaults for a fresh install.
...
`language` (`"fr"` or `"en"`, default `"fr"`), `app_title`,
`allow_keyword_changes` (`true`/`false`, default `true`), `enable_debug`
(`true`/`false`, default `false`), and `options_password` (default `"baloney"`).
